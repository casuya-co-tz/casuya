"""Bounds checking for STT uploads (mono 16-bit PCM WAV)."""
...
def validate_wav(pcm16_wav: bytes) -> tuple[int, float]:
    """Return `(sample_rate, duration_s)` when the blob is a valid bounded WAV.

    Raises `ValueError` when the blob is invalid or exceeds size/duration limits.
    """
